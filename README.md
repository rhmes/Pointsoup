...
- **Training Logs & Plots:** Training logs and plots are now saved in the `logs/` directory for better tracking of model performance over time.
...
│   ├── utils.py
├── model/
│   ├── exp/           # Original model checkpoints
│   ├── exp-sa/        # Updated model checkpoints (with simple attention)
├── data/
│   ├── example_pc_1023/   # Example point clouds
│   ├── compressed/        # Compressed files
│   ├── decompressed/      # Decompressed files
├── csv/                   # Evaluation results (generated by eval_PSNR.py)
└── README.md
```

## Important Paths
- `data/`: Contains input point clouds, compressed, and decompressed files.
- `model/exp/`: Stores model checkpoints for loading in evaluation and training.
- `csv/`: Stores CSV files generated by evaluation scripts.
- `environment/`: Contains environment setup scripts and configuration files, including standard and (Xos) Mac MPS setup scripts (`env_create.sh`, `env_create_mps.sh`) and conda environment files (`environment.yml`, `environment_mps.yml`).
